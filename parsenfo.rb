...
Please, select input file or folder!}
	#Attributes.
	#attr_reader :log_kind, :out_file_kind, :log_file, :in_files,
	...
	def initialize(options, in_file, out_file)
		...
		if out_file.nil?
			@out_file = "#{File.dirname(@in_files.first)}/" +
				"#{OUT_LOG_FILE_NAME}#{out_file_ext}"
		#If out_file has no path stores it in folder with first in_file.
		elsif File.dirname(out_file) == "."
			@out_file = "#{File.dirname(@in_files.first)}/" +
				"#{out_file}#{out_file_ext if File.extname(out_file).empty?}"
		else
			@out_file = "#{out_file}" +
				"#{out_file_ext if File.extname(out_file).empty?}"
		end

		#Defines log file.
		@log_file = "#{File.dirname(__FILE__)}/" +
			"#{OUT_LOG_FILE_NAME}#{LOG_FILE_EXT}"
		#Defines array of log lines.
		@log_lines = ["##### parsenfo.rb log file created at " +
			"#{Time.now} #####\n##### output file: #{@out_file} #####"]
	end

	#Gets lines from each input file and calls external block to treat it.
	#Result of calling (album track records) adds to output array.
	def get_each
...
class Album
	...
	Info = Struct.new(:codec, :album_artist, :album_title, :year,
		:publisher, :genre, :style, :comment, :track_counts,
		:disc_titles, :track_artists, :track_titles, :composers)
	Disc = Struct.new(:disc_number, :disc_title, :tracks)
	Track = Struct.new(:track_number, :track_artist,
		:track_title, :composer)
	...
	def initialize(lines)
		...
		@codec, @album_artist, @album_title, @year, @publisher,
			@genre, @style, @comment, @cover, @discs =
		...
		while disc_count > i
			...
			#Number of tracks of current disc is whether stored in MTC tag
			#of this disc or (for last disc) is calculated as number
			#of all track of album minus sum of all MTS tags.
			track_count = info.track_counts[i].nil? ?
				info.track_titles.length -
				info.track_counts.inject {|sum, n| sum.to_i + n.to_i}.to_i :
	...
	def parse_info(lines)
		...
		info = Info.new(nil, nil, nil, nil, nil, nil, nil, nil,
			[], [], [], [], [])
		lines.each do |line|
			#Finds tag in line, rejects empty lines & lines with no tag.
			next if line.empty? ||
				(tag_end = line.index("=")).nil? || tag_end > 5
			#Detaches tag name.
	...
	#Fills empty records (disc title, track artist & composer) by default.
	def default
		@discs.each do |disc|
			if disc.disc_title.nil?
				disc.disc_title = @discs.length > 1 ?
					"#{@album_title} CD#{disc.disc_number}" : @album_title
			end
			disc.tracks.each do |track|
				track.track_artist = @album_artist if track.track_artist.nil?
				if track.composer.nil?
					if !@discs.first.tracks.first.composer.nil?
						track.composer = @discs.first.tracks.first.composer
...
class RecordsDatabase < SQLite3Database
	...
	YEAR_COLUMN = Column.new("year",
		"INTEGER NOT NULL CHECK ((year > 1900) " +
		"AND (year < SUBSTR(CURRENT_DATE,1,4) + 1)) UNIQUE")
	PUBLISHER_COLUMN = Column.new("publisher",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")
	CODEC_COLUMN = Column.new("codec",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")
	COMMENT_COLUMN = Column.new("comment",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")  
	COVER_COLUMN = Column.new("cover", "BLOB NOT NULL UNIQUE")
	ARTIST_COLUMN = Column.new("artist",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")
	STYLE_COLUMN = Column.new("style",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")
	COMPOSER_COLUMN = Column.new("composer",
		"TEXT NOT NULL UNIQUE COLLATE NOCASE")
	...
	DISC_NUMBER_COLUMN = Column.new("disc_number",
		"INTEGER NOT NULL CHECK (disc_number > 0)")
	TRACK_NUMBER_COLUMN = Column.new("track_number",
		"INTEGER NOT NULL CHECK (track_number > 0)")
	...
	COMPOSER_ORDER_COLUMN = Column.new("composer_order",
		"INTEGER NOT NULL")
	...
	ALBUMS_TABLE = Table.new("albums", nil,
		[TITLE_ID_COLUMN, YEAR_ID_COLUMN, PUBLISHER_ID_COLUMN,
		CODEC_ID_COLUMN, COMMENT_ID_COLUMN, COVER_ID_COLUMN])
	TRACKS_TABLE = Table.new("tracks",
		"UNIQUE (#{TRACK_NUMBER_COLUMN.header}, " +
		"#{DISC_NUMBER_COLUMN.header}, #{ALBUM_ID_COLUMN.header})",
		[TITLE_ID_COLUMN, TRACK_NUMBER_COLUMN, DISC_NUMBER_COLUMN,
		ALBUM_ID_COLUMN])

	#Junction tables.
	ALBUMS_ARTISTS_TABLE = Table.new("albums_artists",
		"UNIQUE (#{ALBUM_ID_COLUMN.header}, #{ARTIST_ID_COLUMN.header})",
		[ALBUM_ID_COLUMN, ARTIST_ID_COLUMN, ARTIST_ORDER_COLUMN])
	ALBUMS_STYLES_TABLE = Table.new("albums_styles",
		"UNIQUE (#{ALBUM_ID_COLUMN.header}, #{STYLE_ID_COLUMN.header})",
		[ALBUM_ID_COLUMN, STYLE_ID_COLUMN, STYLE_ORDER_COLUMN])
	TRACKS_ARTISTS_TABLE = Table.new("tracks_artists",
		"UNIQUE (#{TRACK_ID_COLUMN.header}, #{ARTIST_ID_COLUMN.header})",
		[TRACK_ID_COLUMN, ARTIST_ID_COLUMN, ARTIST_ORDER_COLUMN])
	TRACKS_COMPOSERS_TABLE = Table.new("tracks_composers",
		"UNIQUE (#{TRACK_ID_COLUMN.header}, #{COMPOSER_ID_COLUMN.header})",
		[TRACK_ID_COLUMN, COMPOSER_ID_COLUMN, COMPOSER_ORDER_COLUMN])

	#Database definition.
	TABLES = [TITLES_TABLE, YEARS_TABLE, PUBLISHERS_TABLE, CODECS_TABLE,
		COMMENTS_TABLE, COVERS_TABLE, STYLES_TABLE, COMPOSERS_TABLE,
	...
	#For every track inserts record into corresponding table.
